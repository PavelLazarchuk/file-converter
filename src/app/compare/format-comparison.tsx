...
export function FormatComparison({ files, quality, onDismiss }: ComparisonProps) {
    ...
    return (
        <div className="space-y-4 rounded-xl border bg-card p-4 motion-reduce:animate-none animate-in duration-300 ease-out fade-in-0 zoom-in-95 slide-in-from-bottom-3">
                ...
                <div className="flex items-start gap-2.5">
                    ...
                    <div>
                        <p className="text-sm font-medium">
                            {best.label} is smallest — {formatFileSize(best.size)}
                        </p>
                        <p className="text-sm text-muted-foreground">
                            Encoded at quality {quality} from a {formatFileSize(originalSize)}{' '}
                            original. Same pixels in every row.
                        </p>
                    </div>
                </div>
                <Button
                    ...
                    return (
                        ...
                        >
                            ...
                            <Button
                                ...
                                onClick={() => downloadResult(row.file)}
                            >
                                <Download />
                            </Button>
                        </li>
            ...
            <p className="text-sm text-muted-foreground">
                Smaller is not automatically better: AVIF and WEBP are unsupported in some older
                software, and PNG is the only lossless row here.
            </p>
        </div>
    );
}
