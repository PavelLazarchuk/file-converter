        ...
        <form onSubmit={onSubmit} className="space-y-6" noValidate>
            ...
            <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                    Every format is encoded at this same setting, which is what makes the sizes
                    comparable. The numbers match what the Compress tool produces.
                </p>
                {errors.quality && (
            ...
            <Button type="submit" className="w-full" disabled={!hasImage || !isValid || isPending}>
                {isPending ? (
                    <>
                        <Spinner /> Encoding {COMPARE_FORMAT_KEYS.length} formats…
                    </>
                ) : (
                    'Compare formats'
                )}
